...
@section('content')
    ...
    @php
        $expenseWithAttachmentPaidQuickFilterQuery = array_merge(request()->query(), [
            'has_attachment' => '1',
            'payment_status' => 'paid',
        ]);
    @endphp
    ...
    <div
        class="card"
        data-testid="expense-with-attachment-paid-quick-filter-card"
        data-quick-filter-card="expense"
        data-quick-filter-style="unified"
        style="margin-bottom:20px;border-color:#d1d5db;background:#ffffff;"
    >
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap;">
            <div>
                <h2 style="margin-top:0;">مصروفات بها مرفق ومدفوعة</h2>
                <div class="muted">
                    استخدم هذا الفلتر السريع لعرض المصروفات التي تحتوي على مرفق وتم سدادها.
                </div>
            </div>

            <div>
                <a
                    href="{{ route('expenses.index', $expenseWithAttachmentPaidQuickFilterQuery) }}"
                    class="btn secondary"
                    data-testid="expense-with-attachment-paid-quick-filter"
                >
                    عرض المصروفات بها مرفق والمدفوعة
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="card" data-testid="expense-large-paid-summary" style="margin-bottom:20px;border-color:#b7e4c7;background:#f7fff9;">
        <div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap;">
            <div>
                ...
                <div style="margin-top:12px;">
                    <a
                        href="{{ route('expenses.export-large-paid', array_merge(request()->except('page'), ['large_amount' => '1', 'payment_status' => 'paid'])) }}"
                        class="btn"
                        data-testid="expense-large-paid-summary-export"
                    >
                        تصدير CSV للمصاريف الكبيرة المدفوعة
                    </a>
                </div>
            </div>
