...
<head>
    <meta charset="utf-8">
    <title>طباعة تقرير أعمار ذمم الموردين</title>
    <style>
        body {
...
<body>
    <div class="actions">
        <button type="button" class="button" onclick="window.print()" data-testid="supplier-aging-print-button">طباعة</button>
    </div>

    <div class="header">
        <h1>تقرير أعمار ذمم الموردين</h1>
        <div class="meta">تاريخ التقرير: {{ $reportDate->format('Y-m-d') }}</div>
        <div class="meta">فلتر المورد: {{ $supplierFilterLabel }}</div>
        <div class="meta">فلتر شريحة العمر: {{ $agingBucketFilterLabel }}</div>
    </div>

    <div class="summary" data-testid="supplier-aging-print-summary">
        <div class="card">
    ...
    @if ($rows->isEmpty())
        <div class="empty" data-testid="supplier-aging-print-empty">لا توجد ذمم مفتوحة للموردين.</div>
    @else
        <table data-testid="supplier-aging-print-table">
            <thead>
                <tr>
                    <th>المورد</th>
                    <th>عدد الفواتير</th>
