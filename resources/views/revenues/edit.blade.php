...
<div class="page">
    <div class="card">
        <h1 style="margin-top:0;">تعديل إيراد</h1>
        <div class="muted">
            تعديل بيانات الإيراد رقم {{ $revenue->code ?? $revenue->id }}.
        </div>
    </div>

    <div class="card" data-testid="revenue-edit-form-card">
        <form method="POST" action="{{ route('revenues.update', $revenue) }}" data-testid="revenue-edit-form">
            @csrf
            @method('PUT')
            ...
            <div class="grid">
                <div>
                    ...
                    <select name="branch_id" id="branch_id" required>
                        ...
                        @foreach ($branches as $branch)
                            <option value="{{ $branch->id }}" @selected((string) old('branch_id', $revenue->branch_id) === (string) $branch->id)>
                ...
                <div>
                    ...
                    <select name="revenue_category_id" id="revenue_category_id" required>
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" @selected((string) old('revenue_category_id', $revenue->revenue_category_id) === (string) $category->id)>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="revenue_date" id="revenue_date" value="{{ old('revenue_date', $revenue->revenue_date?->format('Y-m-d')) }}" required>
                    @error('revenue_date') <div class="error">{{ $message }}</div> @enderror
                </div>

                <div>
                    <label for="amount">المبلغ</label>
                    <input type="number" step="0.01" min="0.01" name="amount" id="amount" value="{{ old('amount', $revenue->amount) }}" required>
                    @error('amount') <div class="error">{{ $message }}</div> @enderror
                </div>

                <div>
                    <label for="tax_amount">الضريبة</label>
                    <input type="number" step="0.01" min="0" name="tax_amount" id="tax_amount" value="{{ old('tax_amount', $revenue->tax_amount) }}">
                ...
                <div>
                    ...
                    <select name="collection_method" id="collection_method" required>
                        @foreach ($collectionMethods as $key => $label)
                            <option value="{{ $key }}" @selected(old('collection_method', $revenue->collection_method) === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="collection_status" id="collection_status" required>
                        @foreach ($collectionStatuses as $key => $label)
                            <option value="{{ $key }}" @selected(old('collection_status', $revenue->is_collected ? 'collected' : 'uncollected') === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="reference_number" id="reference_number" value="{{ old('reference_number', $revenue->reference_number) }}">
            ...
            <div style="margin-top:14px;">
                ...
                <input type="text" name="description" id="description" value="{{ old('description', $revenue->description) }}" required>
                @error('description') <div class="error">{{ $message }}</div> @enderror
            </div>

            <div style="margin-top:14px;">
                <label for="notes">ملاحظات</label>
                <textarea name="notes" id="notes">{{ old('notes', $revenue->notes) }}</textarea>
