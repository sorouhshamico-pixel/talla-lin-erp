...
class SupplierPurchaseInvoiceAgingDrilldownFilterPreferenceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_supplier_aging_drilldown_saves_submitted_filters_as_user_preferences(): void
    {
    ...
    public function test_supplier_aging_drilldown_reuses_saved_filter_preferences_when_no_filters_are_submitted(): void
    {
        $user = User::query()->firstOrFail();
        $supplier = Supplier::query()->orderBy('id')->firstOrFail();
        $branchId = (int) DB::table('branches')->orderBy('id')->value('id');
        ...
        $response->assertSee(e(route('reports.supplier-purchase-invoice-aging.drilldown.export', [
            'supplier_id' => $supplier->id,
            'branch_id' => $branchId,
            'as_of_date' => '2026-07-31',
            'aging_bucket' => 'without_due_date',
    ...
    public function test_supplier_aging_drilldown_reset_clears_saved_filter_preferences(): void
    {
        $user = User::query()->firstOrFail();
        $supplier = Supplier::query()->orderBy('id')->firstOrFail();
        $branchId = (int) DB::table('branches')->orderBy('id')->value('id');
        ...
        $response->assertSee(e(route('reports.supplier-purchase-invoice-aging.drilldown.export')), false);
    }

    public function test_supplier_aging_drilldown_export_can_use_saved_filter_preferences(): void
    {
        $user = User::query()->firstOrFail();
        $supplier = Supplier::query()->orderBy('id')->firstOrFail();
        $branchId = (int) DB::table('branches')->orderBy('id')->value('id');
        ...
        $exportResponse = $this->actingAs($user)->get(route('reports.supplier-purchase-invoice-aging.drilldown.export'));
