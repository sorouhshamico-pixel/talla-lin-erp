...
class RevenueFilteredExportTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_revenues_can_be_exported_to_csv_using_current_filters(): void
    {
        $this->user = User::factory()->create();

        $this->actingAs($this->user);

        $uncollectedLabel = 'filtered-export-uncollected-12j-' . uniqid();
        $collectedLabel = 'filtered-export-collected-12j-' . uniqid();

        $this->assertNotNull(
            $this->labelColumn(),
            'Revenue filtered export test requires a searchable label column such as description, notes, reference, title, or name.'
        );

        $this->insertRevenueForExportTest(
        ...
        $response = $this->get(route('revenues.export', [
            'is_collected' => 0,
        ]));
        ...
        $this->assertStringContainsString('revenues-', $contentDisposition);
        $this->assertStringContainsString('.csv', $contentDisposition);

        $csv = $response->streamedContent();

        $this->assertNotSame('', trim($csv));
        $this->assertStringContainsString($uncollectedLabel, $csv);
        $this->assertStringNotContainsString($collectedLabel, $csv);
    }
