...
class ReportSavedViewPhase64HNextTargetLockTest extends TestCase
{
    public function test_phase_64h_lock_files_exist(): void
    {
        $this->assertFileExists(base_path('docs/phase-64h-saved-view-rollout-selector-snapshot.json'));
        $this->assertFileExists(base_path('docs/phase-64h-next-saved-view-rollout-target.json'));
        $this->assertFileExists(base_path('docs/phase-64h-next-saved-view-rollout-target.md'));
    }

    public function test_phase_64h_locked_target_matches_selector_snapshot(): void
    {
        $snapshot = json_decode(
            file_get_contents(base_path('docs/phase-64h-saved-view-rollout-selector-snapshot.json')),
            true
        );
        ...
        $this->assertSame('Phase 64H', $snapshot['phase']);
        $this->assertSame('Phase 64H', $lock['phase']);
    ...
    public function test_phase_64h_locked_target_view_exists_and_config_path_uses_convention(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64h-next-saved-view-rollout-target.json')),
            true
    ...
    public function test_phase_64h_lock_is_documented(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64h-next-saved-view-rollout-target.json')),
            true
        ...
        $doc = file_get_contents(base_path('docs/phase-64h-next-saved-view-rollout-target.md'));

        $this->assertStringContainsString('Phase 64H', $doc);
        $this->assertStringContainsString('Phase 64G clean', $doc);
        $this->assertStringContainsString('23006ae', $doc);
