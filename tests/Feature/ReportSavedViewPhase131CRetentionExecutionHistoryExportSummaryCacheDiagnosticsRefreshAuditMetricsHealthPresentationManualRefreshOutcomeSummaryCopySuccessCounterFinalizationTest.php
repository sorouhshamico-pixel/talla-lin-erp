...
class ReportSavedViewPhase131CRetentionExecutionHistoryExportSummaryCacheDiagnosticsRefreshAuditMetricsHealthPresentationManualRefreshOutcomeSummaryCopySuccessCounterFinalizationTest
    extends TestCase
{
    private const JSON_PATH =
        'docs/'
        . 'phase-131c-retention-execution-history-export-summary-cache-diagnostics-refresh-audit-metrics-health-presentation-manual-refresh-outcome-summary-copy-success-counter-finalization.json';
    ...
    public function test_finalization_documents_and_baseline_are_locked(): void
    {
        ...
        $this->assertFileExists(base_path(
            'docs/'
            . 'phase-131c-retention-execution-history-export-summary-cache-diagnostics-refresh-audit-metrics-health-presentation-manual-refresh-outcome-summary-copy-success-counter-finalization.md'
        ));

        $document = $this->document();

        $this->assertSame('Phase 131C', $document['phase']);
        $this->assertSame('finalization', $document['type']);
        $this->assertSame(
            'aea6522536c67dd47a1220b8153885473eacdf2e',
            $document['baseline']['commit']
        );
        $this->assertSame(2518, $document['baseline']['tests']);
        $this->assertSame(26332, $document['baseline']['assertions']);
    }
    ...
    public function test_success_counter_state_and_counting_rules_are_locked(): void
    {
    ...
    public function test_render_attempt_copy_availability_and_legacy_contract_are_locked(): void
    {
        $locked = $this->document()['locked_implementation'];
        ...
        $this->assertTrue(
            $locked['legacy_contract']['phase_130b_attempt_counter_preserved']
        );
    }
    ...
    public function test_compatibility_scope_workflow_and_next_phase_are_locked(): void
    {
        ...
        $this->assertTrue(
            $locked['implementation_scope']['phase_131b_test_added']
        );
        $this->assertSame(
            2,
