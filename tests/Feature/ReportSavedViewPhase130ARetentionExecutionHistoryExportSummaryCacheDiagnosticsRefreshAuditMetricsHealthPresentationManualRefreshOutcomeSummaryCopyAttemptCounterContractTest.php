...
class ReportSavedViewPhase130ARetentionExecutionHistoryExportSummaryCacheDiagnosticsRefreshAuditMetricsHealthPresentationManualRefreshOutcomeSummaryCopyAttemptCounterContractTest
    extends TestCase
{
    private const JSON_PATH =
        'docs/'
        . 'phase-130a-retention-execution-history-export-summary-cache-diagnostics-refresh-audit-metrics-health-presentation-manual-refresh-outcome-summary-copy-attempt-counter-contract.json';
    ...
    public function test_contract_documents_and_baseline_are_locked(): void
    {
        ...
        $this->assertFileExists(base_path(
            'docs/'
            . 'phase-130a-retention-execution-history-export-summary-cache-diagnostics-refresh-audit-metrics-health-presentation-manual-refresh-outcome-summary-copy-attempt-counter-contract.md'
        ));

        $document = $this->document();

        $this->assertSame('Phase 130A', $document['phase']);
        $this->assertSame('contract', $document['type']);
        $this->assertSame(
            '965295dc338301b24c35200781bca5fff8de73b3',
            $document['baseline']['commit']
        );
        $this->assertSame(2489, $document['baseline']['tests']);
        $this->assertSame(25921, $document['baseline']['assertions']);
    }
    ...
    {
        $contract = $this->document()['copy_attempt_counter_contract'];
        ...
        $this->assertSame(
            'manualRefreshOutcomeSummaryCopyAttempts',
    ...
    public function test_counting_render_and_copy_rules_are_locked(): void
    {
        $contract = $this->document()['copy_attempt_counter_contract'];

        foreach ([
            'count_explicit_supported_copy_attempt',
            'count_resolved_copy',
            'count_rejected_copy',
            'increment_before_clipboard_write',
            'increment_once_per_eligible_click',
        ] as $key) {
    ...
    {
        $document = $this->document();
        $contract = $document['copy_attempt_counter_contract'];
        ...
        $this->assertTrue(
            $contract['legacy_contract']['phase_129b_status_reset_preserved']
        );
