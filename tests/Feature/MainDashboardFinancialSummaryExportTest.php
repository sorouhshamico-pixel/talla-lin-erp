...
class MainDashboardFinancialSummaryExportTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_main_dashboard_displays_financial_summary_export_link(): void
    {
        ...
        $response->assertOk();
        $response->assertSee('data-testid="main-dashboard-financial-export-link"', false);
        $response->assertSee(route('dashboard.financial-summary.export'), false);
        $response->assertSee('تصدير الملخص CSV');
    }

    public function test_main_dashboard_financial_summary_export_contains_summary_and_risk_indicators(): void
    {
        $user = User::query()->firstOrFail();
        ...
        $customer = $this->createCustomer([
            'name' => 'عميل تصدير ملخص الصفحة الرئيسية',
            'phone' => '0579853031',
        ]);

        $supplier = $this->createSupplier([
            'name' => 'مورد تصدير ملخص الصفحة الرئيسية',
            'phone' => '0579853032',
        ]);

        $this->createSalesInvoice([
            'customer_id' => $customer->id,
            'invoice_number' => 'SI-MAIN-EXPORT-001',
        ...
        $this->createPurchaseInvoice([
            ...
            'invoice_number' => 'PI-MAIN-EXPORT-001',
        ...
        $this->assertStringContainsString('الملخص المالي السريع للوحة التحكم', $content);
        $this->assertStringContainsString('ذمم العملاء المفتوحة', $content);
        $this->assertStringContainsString('1000.00', $content);
        $this->assertStringContainsString('التزامات الموردين المفتوحة', $content);
        $this->assertStringContainsString('3000.00', $content);
        $this->assertStringContainsString('صافي التدفق النقدي المتوقع', $content);
        $this->assertStringContainsString('-2000.00', $content);
        $this->assertStringContainsString('صافي الضغط النقدي المتأخر', $content);
        $this->assertStringContainsString('2000.00', $content);
        $this->assertStringContainsString('33.33%', $content);
        $this->assertStringContainsString('تغطية نقدية متوقعة غير كافية', $content);
        $this->assertStringContainsString('يتطلب متابعة نقدية', $content);
    }
    ...
    private function createCustomer(array $overrides = []): Customer
    {
        ...
        $data = array_merge([
            ...
            'name' => 'عميل اختبار تصدير ملخص الصفحة الرئيسية',
            'phone' => '0579853000',
            'email' => uniqid('main-dashboard-export-customer-') . '@example.com',
            'city' => 'الرياض',
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
    ...
    private function createSupplier(array $overrides = []): Supplier
    {
        ...
        $data = array_merge([
            'company_id' => (int) DB::table('companies')->value('id'),
            'name' => 'مورد اختبار تصدير ملخص الصفحة الرئيسية',
            'phone' => '0579853001',
            'email' => uniqid('main-dashboard-export-supplier-') . '@example.com',
            'city' => 'الرياض',
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
    ...
    private function createSalesInvoice(array $overrides = []): SalesInvoice
    {
        ...
        $data = array_merge([
            ...
            'invoice_number' => uniqid('SI-MAIN-EXPORT-'),
            'status' => 'issued',
    ...
    private function createPurchaseInvoice(array $overrides = []): PurchaseInvoice
    {
        ...
        $data = array_merge([
            ...
            'supplier_id' => null,
            'user_id' => (int) DB::table('users')->value('id'),
            'invoice_number' => uniqid('PI-MAIN-EXPORT-'),
