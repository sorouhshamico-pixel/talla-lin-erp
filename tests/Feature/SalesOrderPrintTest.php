...
class SalesOrderPrintTest extends TestCase
{
    use RefreshDatabase;

    public function test_sales_order_print_page_can_be_viewed(): void
    {
        $user = User::factory()->create();

        $customer = Customer::create([
            'company_id' => $this->createCompanyId(),
            'name' => 'عميل طباعة أمر البيع',
            'phone' => '0504040404',
            'email' => 'sales-order-print@example.com',
            'address' => 'الرياض',
            'is_active' => true,
        ]);
        ...
        $quotation = Quotation::create([
            ...
            'status' => 'accepted',
            'total_amount' => 2500,
            'notes' => 'عرض سعر مرتبط بطباعة أمر البيع',
        ]);
        ...
        $response = $this->actingAs($user)->get('/sales-orders/' . $salesOrder->id . '/print');

        $response->assertOk();
        $response->assertSee('طباعة أمر البيع');
        $response->assertSee('SO-000001');
        $response->assertSee('QT-000001');
        $response->assertSee('عميل طباعة أمر البيع');
