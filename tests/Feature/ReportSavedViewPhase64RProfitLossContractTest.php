...
class ReportSavedViewPhase64RProfitLossContractTest extends TestCase
{
    public function test_phase_64r_contract_files_exist(): void
    {
        $this->assertFileExists(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.json'));
        $this->assertFileExists(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.md'));
    }

    public function test_phase_64r_contract_matches_phase_64q_locked_target(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64q-next-saved-view-rollout-target.json')),
            true
        );

        $contract = json_decode(
            file_get_contents(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.json')),
            true
        );

        $this->assertSame('Phase 64R', $contract['phase']);
        $this->assertSame('profit-loss', $contract['target']['key']);
    ...
    public function test_phase_64r_contract_target_view_controller_and_routes_exist(): void
    {
        $contract = json_decode(
            file_get_contents(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.json')),
            true
        ...
        $this->assertTrue(Route::has('reports.profit-loss'));
        $this->assertTrue(Route::has('reports.profit-loss.export'));
        ...
        $this->assertTrue($contract['current_state_evidence']['controller_has_invoke_method']);
    ...
    public function test_phase_64r_contract_uses_saved_view_controls_conventions(): void
    {
        $contract = json_decode(
            file_get_contents(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.json')),
            true
        ...
        $this->assertSame('profit-loss', $registryKey);
    ...
    public function test_phase_64r_contract_documents_routes_hidden_fields_and_test_ids(): void
    {
        $contract = json_decode(
            file_get_contents(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.json')),
            true
        ...
        $this->assertSame('reports.profit-loss', $contract['contract']['index_route']);
        $this->assertSame('reports.profit-loss.export', $contract['contract']['export_route']);
        $this->assertSame('reports.profit-loss.saved-views.store', $contract['contract']['saved_view_store_route']);
    ...
    public function test_phase_64r_contract_is_documented(): void
    {
        $contract = json_decode(
            file_get_contents(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.json')),
            true
        ...
        $doc = file_get_contents(base_path('docs/phase-64r-profit-loss-saved-view-controls-contract.md'));

        $this->assertStringContainsString('Phase 64R', $doc);
        $this->assertStringContainsString('profit-loss', $doc);
        ...
        $this->assertStringContainsString('ReportSavedViewPhase64RProfitLossContractTest', $doc);
    }
}
