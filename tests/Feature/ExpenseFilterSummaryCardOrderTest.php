...
class ExpenseFilterSummaryCardOrderTest extends TestCase
{
    use RefreshDatabase;

    public function test_expense_filter_summary_cards_are_rendered_in_expected_order_when_filters_exist(): void
    {
        ...
        $this->assertStringContainsString('data-page-active-filter-count="2"', $content);
        $this->assertStringContainsString('data-active-filter-alert-count="2"', $content);
        $this->assertStringContainsString('data-active-filter-count="2"', $content);
    ...
    public function test_expense_filter_summary_cards_keep_expected_idle_state_without_real_filters(): void
    {
        $this->actingAs(User::factory()->create());
