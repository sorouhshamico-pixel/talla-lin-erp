...
class SalesInvoiceAgingReportDefaultSavedViewTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_sales_invoice_aging_report_applies_default_saved_view_when_opened_without_filters(): void
    {
        ...
        ReportSavedView::query()->create([
            ...
            'name' => 'عرض افتراضي لفواتير المبيعات',
            'filters' => [
    ...
    public function test_sales_invoice_aging_report_explicit_filters_override_default_saved_view(): void
    {
        $user = User::query()->firstOrFail();
        $customer = Customer::query()->orderBy('id')->firstOrFail();
        ...
        $this->assertArrayNotHasKey('customer_id', array_filter($filters, fn ($value) => $value !== null && $value !== ''));
    }

    public function test_sales_invoice_aging_report_reset_filters_does_not_apply_default_saved_view(): void
    {
        $user = User::query()->firstOrFail();
        $customer = Customer::query()->orderBy('id')->firstOrFail();
