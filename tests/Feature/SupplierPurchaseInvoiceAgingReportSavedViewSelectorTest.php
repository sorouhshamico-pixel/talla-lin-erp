...
class SupplierPurchaseInvoiceAgingReportSavedViewSelectorTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_supplier_aging_report_shows_empty_saved_views_selector(): void
    {
    ...
    public function test_supplier_aging_report_lists_saved_views_for_current_report(): void
    {
        $user = User::query()->firstOrFail();
        ...
        $response->assertOk();
        $response->assertSee('data-testid="supplier-aging-saved-views-list"', false);
        $response->assertSee('data-testid="supplier-aging-saved-view-open-link"', false);
        $response->assertSee('عرض افتراضي لذمم الموردين');
        $response->assertSee('data-testid="supplier-aging-saved-view-default-badge"', false);
