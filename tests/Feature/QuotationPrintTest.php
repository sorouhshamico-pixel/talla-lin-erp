...
class QuotationPrintTest extends TestCase
{
    ...
    public function test_quotation_print_page_can_be_viewed(): void
    {
        $user = User::factory()->create();

        $customer = Customer::create([
            'company_id' => $this->createCompanyId(),
            'name' => 'عميل طباعة عرض السعر',
            'phone' => '0588888888',
            'email' => 'quotation-print@example.com',
            'address' => 'الرياض',
            'is_active' => true,
        ]);
        ...
        $response->assertOk();
        $response->assertSee('طباعة عرض السعر');
        $response->assertSee('QT-000001');
        $response->assertSee('عميل طباعة عرض السعر');
