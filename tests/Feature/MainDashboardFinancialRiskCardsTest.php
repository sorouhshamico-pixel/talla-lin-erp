...
class MainDashboardFinancialRiskCardsTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_financial_dashboard_summary_service_calculates_risk_indicators(): void
    {
        SalesInvoice::query()->delete();
        PurchaseInvoice::query()->delete();

        $customer = $this->createCustomer([
            'name' => 'عميل مؤشرات المخاطر المالية',
            'phone' => '0579852931',
        ]);

        $supplier = $this->createSupplier([
            'name' => 'مورد مؤشرات المخاطر المالية',
            'phone' => '0579852932',
        ]);

        $this->createSalesInvoice([
            'customer_id' => $customer->id,
            'invoice_number' => 'SI-MAIN-RISK-001',
        ...
        $this->createPurchaseInvoice([
            ...
            'invoice_number' => 'PI-MAIN-RISK-001',
        ...
        $this->assertSame(1000.00, $summary['expected_inflows']);
        $this->assertSame(3000.00, $summary['expected_outflows']);
        $this->assertSame(-2000.00, $summary['net_expected_cash']);
        $this->assertSame(2000.00, $summary['net_overdue_pressure']);
        $this->assertSame(33.33, $summary['cash_coverage_ratio']);
        $this->assertSame('تغطية نقدية متوقعة غير كافية', $summary['cash_coverage_label']);
        $this->assertSame('يتطلب متابعة نقدية', $summary['risk_label']);
    }

    public function test_main_dashboard_displays_financial_risk_cards_and_drilldown_links(): void
    {
        ...
        $response->assertOk();
        $response->assertSee('data-testid="main-dashboard-financial-risk-cards"', false);
        $response->assertSee('صافي الضغط النقدي المتأخر');
        $response->assertSee('نسبة تغطية الالتزامات');
        $response->assertSee('حالة التغطية النقدية');
        $response->assertSee('مؤشر المتابعة المالية');
        $response->assertSee('data-testid="main-dashboard-customer-drilldown-link"', false);
        $response->assertSee('data-testid="main-dashboard-supplier-drilldown-link"', false);
    ...
    private function createCustomer(array $overrides = []): Customer
    {
        ...
        $data = array_merge([
            ...
            'name' => 'عميل اختبار مؤشرات المخاطر المالية',
            'phone' => '0579852900',
            'email' => uniqid('main-dashboard-risk-customer-') . '@example.com',
            'city' => 'الرياض',
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
    ...
    private function createSupplier(array $overrides = []): Supplier
    {
        ...
        $data = array_merge([
            'company_id' => (int) DB::table('companies')->value('id'),
            'name' => 'مورد اختبار مؤشرات المخاطر المالية',
            'phone' => '0579852901',
            'email' => uniqid('main-dashboard-risk-supplier-') . '@example.com',
            'city' => 'الرياض',
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
    ...
    private function createSalesInvoice(array $overrides = []): SalesInvoice
    {
        ...
        $data = array_merge([
            ...
            'invoice_number' => uniqid('SI-MAIN-RISK-'),
            'status' => 'issued',
    ...
    private function createPurchaseInvoice(array $overrides = []): PurchaseInvoice
    {
        ...
        $data = array_merge([
            ...
            'supplier_id' => null,
            'user_id' => (int) DB::table('users')->value('id'),
            'invoice_number' => uniqid('PI-MAIN-RISK-'),
