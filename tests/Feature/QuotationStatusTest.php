...
class QuotationStatusTest extends TestCase
{
    ...
    public function test_user_can_update_quotation_status(): void
    {
        ...
        $quotation = Quotation::create([
            ...
            'status' => 'draft',
            'total_amount' => 0,
            'notes' => 'اختبار تغيير الحالة',
        ]);

        $response = $this->actingAs($user)->patch('/quotations/' . $quotation->id . '/status', [
            'status' => 'sent',
        ]);
    ...
    public function test_invalid_quotation_status_is_rejected(): void
    {
        $user = User::factory()->create();

        $customer = Customer::create([
            'company_id' => $this->createCompanyId(),
            'name' => 'عميل حالة غير صحيحة',
            'phone' => '0577777777',
            'email' => 'quotation-invalid-status@example.com',
            'address' => 'الرياض',
            'is_active' => true,
        ]);
        ...
        $quotation = Quotation::create([
            ...
            'status' => 'draft',
            'total_amount' => 0,
            'notes' => 'اختبار رفض حالة غير صحيحة',
        ]);

        $response = $this->actingAs($user)->from('/quotations/' . $quotation->id)->patch('/quotations/' . $quotation->id . '/status', [
            'status' => 'wrong-status',
        ]);
