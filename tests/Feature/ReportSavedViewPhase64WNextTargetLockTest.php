...
class ReportSavedViewPhase64WNextTargetLockTest extends TestCase
{
    public function test_phase_64w_lock_files_exist(): void
    {
        $this->assertFileExists(base_path('docs/phase-64w-saved-view-rollout-selector-snapshot.json'));
        $this->assertFileExists(base_path('docs/phase-64w-next-saved-view-rollout-target.json'));
        $this->assertFileExists(base_path('docs/phase-64w-next-saved-view-rollout-target.md'));
    }

    public function test_phase_64w_locked_target_matches_selector_snapshot(): void
    {
        $snapshot = json_decode(
            file_get_contents(base_path('docs/phase-64w-saved-view-rollout-selector-snapshot.json')),
            true
        );
        ...
        $this->assertSame('Phase 64W', $snapshot['phase']);
        $this->assertSame('Phase 64W', $lock['phase']);
    ...
    public function test_phase_64w_locked_target_view_exists_and_config_path_uses_convention(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64w-next-saved-view-rollout-target.json')),
            true
    ...
    public function test_phase_64w_lock_is_documented(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64w-next-saved-view-rollout-target.json')),
            true
        ...
        $doc = file_get_contents(base_path('docs/phase-64w-next-saved-view-rollout-target.md'));

        $this->assertStringContainsString('Phase 64W', $doc);
        $this->assertStringContainsString('Phase 64V clean', $doc);
        $this->assertStringContainsString('a2a2777', $doc);
