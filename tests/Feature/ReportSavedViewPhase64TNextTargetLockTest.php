...
class ReportSavedViewPhase64TNextTargetLockTest extends TestCase
{
    public function test_phase_64t_lock_files_exist(): void
    {
        $this->assertFileExists(base_path('docs/phase-64t-saved-view-rollout-selector-snapshot.json'));
        $this->assertFileExists(base_path('docs/phase-64t-next-saved-view-rollout-target.json'));
        $this->assertFileExists(base_path('docs/phase-64t-next-saved-view-rollout-target.md'));
    }

    public function test_phase_64t_locked_target_matches_selector_snapshot(): void
    {
        $snapshot = json_decode(
            file_get_contents(base_path('docs/phase-64t-saved-view-rollout-selector-snapshot.json')),
            true
        );
        ...
        $this->assertSame('Phase 64T', $snapshot['phase']);
        $this->assertSame('Phase 64T', $lock['phase']);
    ...
    public function test_phase_64t_locked_target_view_exists_and_config_path_uses_convention(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64t-next-saved-view-rollout-target.json')),
            true
    ...
    public function test_phase_64t_lock_is_documented(): void
    {
        $lock = json_decode(
            file_get_contents(base_path('docs/phase-64t-next-saved-view-rollout-target.json')),
            true
        ...
        $doc = file_get_contents(base_path('docs/phase-64t-next-saved-view-rollout-target.md'));

        $this->assertStringContainsString('Phase 64T', $doc);
        $this->assertStringContainsString('Phase 64S clean', $doc);
        $this->assertStringContainsString('6016ab9', $doc);
