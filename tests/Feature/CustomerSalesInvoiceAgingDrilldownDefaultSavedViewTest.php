...
class CustomerSalesInvoiceAgingDrilldownDefaultSavedViewTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_customer_aging_drilldown_applies_default_saved_view_when_opened_without_filters(): void
    {
        ...
        $preference = DB::table('user_report_filter_preferences')
            ...
            ->where('report_key', 'customer-sales-invoice-aging-drilldown')
            ->first();
    ...
    public function test_customer_aging_drilldown_explicit_filters_override_default_saved_view(): void
    {
        $user = User::query()->firstOrFail();
        $customer = Customer::query()->orderBy('id')->firstOrFail();
        $branchId = (int) DB::table('branches')->orderBy('id')->value('id');
    ...
    public function test_customer_aging_drilldown_reset_filters_does_not_apply_default_saved_view(): void
    {
        $user = User::query()->firstOrFail();
        $customer = Customer::query()->orderBy('id')->firstOrFail();
        $branchId = (int) DB::table('branches')->orderBy('id')->value('id');
