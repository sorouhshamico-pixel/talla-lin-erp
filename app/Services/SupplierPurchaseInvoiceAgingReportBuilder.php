...
class SupplierPurchaseInvoiceAgingReportBuilder
{
    public function build(Request $request): array
    {
        ...
        if ($request->filled('supplier_id')) {
            $invoicesQuery->where('supplier_id', $request->input('supplier_id'));
        }
        ...
        $rows = $invoices
            ->groupBy('supplier_id')
            ->map(function ($supplierInvoices) use ($reportDate) {
                $firstInvoice = $supplierInvoices->first();

                $row = [
                    'supplier' => $firstInvoice ? $firstInvoice->supplier : null,
                    'invoice_count' => $supplierInvoices->count(),
                    'remaining_total' => round((float) $supplierInvoices->sum('remaining_amount'), 2),
                    'not_due_total' => 0.0,
        ...
        return [
            ...
            'rows' => $rows,
            'summary' => $summary,
            'supplierFilterLabel' => $this->supplierFilterLabel($request),
            'agingBucketFilterLabel' => $this->agingBucketFilterLabel($request),
        ];
    }

    private function supplierFilterLabel(Request $request): string
    {
        if (! $request->filled('supplier_id')) {
            return 'all';
        }

        $supplier = Supplier::query()->find($request->input('supplier_id'));
